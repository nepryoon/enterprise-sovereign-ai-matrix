...
def test_policy_routes(criticality, sensitivity, expected):
    assert (
        RoutingPolicy()
        .select(RoutingContext(criticality=criticality, sensitivity=sensitivity))
        .model_class
        is expected
    )
...
def test_fast_provider_has_approved_balanced_fallback():
    decision = RoutingPolicy().select(
        RoutingContext(
            criticality=Criticality.LOW, sensitivity=DataSensitivity.PUBLIC, fast_available=False
        )
    )
