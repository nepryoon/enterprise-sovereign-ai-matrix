...
export function useExecutionStream(id: string | null, onEvent: (event: TelemetryEvent) => void, onConnection: (state: "connecting" | "connected" | "reconnecting" | "error") => void) {
  ...
  useEffect(() => {
    ...
    const eventTypes = ["execution.started", "agent.started", "agent.completed", "agent.failed", "agent.message", "routing.selected", "approval.requested", "approval.approved", "approval.rejected", "execution.completed", "execution.failed"];
